...
export default function TypingStage({
  ...
  return (
    <div className="bg-white border border-slate-200/80 shadow-xs rounded-3xl flex-1 flex flex-col items-center justify-center text-center space-y-3 cursor-text relative overflow-hidden p-6 shadow-xs my-2 font-sans select-none">
                ...
                <span className="relative flex items-center justify-center font-black">
                  ...
                  <span
                    className={`absolute inset-0 flex items-center justify-center transition-all duration-100 ${
                       hasError 
                        ? 'text-rose-600' 
                        : (typedCount >= 3 ? 'text-blue-600 font-black' : themeConfig.primaryText)
                    }`}
                    style={clipMaskStyle}
                  >
                    {originalChar === ' ' ? '␣' : originalChar}
