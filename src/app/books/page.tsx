...
import { BookOpen, Sparkles, Bookmark, GraduationCap, FileText } from 'lucide-react';
    ...
    <div className={`flex h-screen ${themeConfig.canvasBg} ${themeConfig.canvasText} overflow-hidden font-sans`}>
      ...
      <div className="flex-1 flex flex-col min-w-0 h-full overflow-hidden">
        ...
        <main className="flex-1 overflow-y-auto p-4 sm:p-6 lg:p-8 space-y-6 pb-28 md:pb-12 max-w-6xl w-full mx-auto [-ms-overflow-style:none] [scrollbar-width:none] [&::-webkit-scrollbar]:hidden">
          {/* CLEAN ELEGANT PAGE HEADER (STATS REMOVED) */}
          <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 border-b border-slate-200/80 pb-4">
            <div className="space-y-1">
              <div className="flex items-center gap-2">
                <span className={`text-[11px] ${themeConfig.primaryText} font-extrabold tracking-wide uppercase flex items-center gap-1.5`}>
                  <BookOpen className="w-3.5 h-3.5" /> 📚 KHO SÁCH &amp; GIÁO TRÌNH LYNKORE
                </span>
              </div>
              ...
              <p className="text-xs sm:text-sm text-slate-500 max-w-2xl leading-relaxed">
                Khám phá trọn bộ 28+ cuốn sách tiếng Hàn từ Sơ cấp đến Cao cấp, TOPIK, Sejong, Yonsei &amp; KIIP.
              </p>
            </div>

            {/* CATEGORY FILTER PILLS */}
            <div className="flex items-center gap-1.5 overflow-x-auto pb-1 sm:pb-0 shrink-0 [-ms-overflow-style:none] [scrollbar-width:none] [&::-webkit-scrollbar]:hidden">
              {categories.map((cat) => (
                <button
                  key={cat}
          ...
          {(selectedCategory === 'Tất cả' || selectedCategory === 'Tiếng Hàn Màu Xanh Lá') && (
            <section className="flex flex-col gap-3">
              <header className="border-l-4 border-[#0076BE] pl-3 flex items-center justify-between">
                <div>
                  <h2 className="font-heading text-xl sm:text-2xl lg:text-3xl text-slate-900 flex items-center gap-2">
                    Tiếng Hàn Màu Xanh Lá
                  </h2>
                  <p className="text-xs sm:text-sm text-slate-500 font-medium">Dành cho người Việt (Luyện thi TOPIK)</p>
                </div>
                <span className="px-2.5 py-0.5 bg-blue-50 text-[#0076BE] border border-blue-200 rounded-full text-xs font-bold">
                  6 Cuốn
                </span>
              </header>
          ...
          {(selectedCategory === 'Tất cả' || selectedCategory === 'Tiếng Hàn Màu Sắc') && (
            <section className="flex flex-col gap-3">
              <header className="border-l-4 border-[#0a6e3a] pl-3 flex items-center justify-between">
                <div>
                  <h2 className="font-heading text-xl sm:text-2xl lg:text-3xl text-slate-900 flex items-center gap-2">
                    <Sparkles className="w-5 h-5 text-emerald-600 inline-block" /> Tiếng Hàn Màu Sắc
                  </h2>
                  <p className="text-xs sm:text-sm text-slate-500 font-medium">Bộ giáo trình 6 cấp độ</p>
                </div>
          ...
          {(selectedCategory === 'Tất cả' || selectedCategory === 'Tiếng Hàn Màu Vàng') && (
            <section className="flex flex-col gap-3">
              <header className="border-l-4 border-[#B27B3E] pl-3 flex items-center justify-between">
                <div>
                  <h2 className="font-heading text-xl sm:text-2xl lg:text-3xl text-slate-900 flex items-center gap-2">
                    <Bookmark className="w-5 h-5 text-[#B27B3E] inline-block" /> Tiếng Hàn Màu Vàng
                  </h2>
                  <p className="text-xs sm:text-sm text-slate-500 font-medium">Giáo trình tiếng Hàn của Vua (Sejong)</p>
                </div>
                <span className="px-2.5 py-0.5 bg-amber-50 text-[#B27B3E] border border-amber-200 rounded-full text-xs font-bold">
                  8 Cuốn
                </span>
              </header>
          ...
          {(selectedCategory === 'Tất cả' || selectedCategory === 'Đại Học Bản Mới') && (
            <section className="flex flex-col gap-3">
              <header className="border-l-4 border-[#E11D2A] pl-3 flex items-center justify-between">
                <div>
                  <h2 className="font-heading text-xl sm:text-2xl lg:text-3xl text-slate-900 flex items-center gap-2">
                    <GraduationCap className="w-5 h-5 text-[#E11D2A] inline-block" /> Tiếng Hàn Đại Học Bản Mới
                  </h2>
                  <p className="text-xs sm:text-sm text-slate-500 font-medium">Trọng tâm từ vựng &amp; ngữ pháp (Yonsei)</p>
                </div>
                <span className="px-2.5 py-0.5 bg-rose-50 text-[#E11D2A] border border-rose-200 rounded-full text-xs font-bold">
                  6 Cuốn
                </span>
              </header>
                  ...
                  <Link key={item.num} href={`/course/yonsei-${item.num}`}>
                    <div className="relative aspect-[3/4] rounded-base border-2 border-border shadow-shadow overflow-hidden transition-all bg-white cursor-pointer hover:translate-x-boxShadowX hover:translate-y-boxShadowY hover:shadow-none">
                      ...
                      <div className="absolute inset-x-0 bottom-[6%] h-[24%]">
                        <div className="w-full h-full transform -scale-y-100">
                          <YonseiBannerSVG />
                        </div>
                      </div>
          ...
          {(selectedCategory === 'Tất cả' || selectedCategory === 'Hội Nhập KIIP') && (
            <section className="flex flex-col gap-3">
              <header className="border-l-4 border-emerald-600 pl-3 flex items-center justify-between">
                <div>
                  <h2 className="font-heading text-xl sm:text-2xl lg:text-3xl text-slate-900 flex items-center gap-2">
                    <FileText className="w-5 h-5 text-emerald-600 inline-block" /> Tiếng Hàn Hội Nhập
                  </h2>
                  <p className="text-xs sm:text-sm text-slate-500 font-medium">Giáo trình tiếng Hàn hội nhập xã hội Hàn Quốc (KIIP)</p>
                </div>
                <span className="px-2.5 py-0.5 bg-emerald-50 text-emerald-700 border border-emerald-200 rounded-full text-xs font-bold">
                  2 Cuốn
                </span>
