...
function SpeakingContent() {
  ...
  const [activeTab, setActiveTab] = useState<SubTab>('pronunciation');
  const [isMobileSidebarOpen, setIsMobileSidebarOpen] = useState(false);

  useEffect(() => {
    if (tabParam && ['shadowing', 'pronunciation', 'roleplay'].includes(tabParam)) {
          ...
          <div className="bg-white border-2 border-slate-900 rounded-3xl p-5 sm:p-6 shadow-xs space-y-4">
            ...
            <div className="flex items-center gap-2 pt-2 border-t border-slate-100 overflow-x-auto [-ms-overflow-style:none] [scrollbar-width:none] [&::-webkit-scrollbar]:hidden">
              ...
              >
                ...
                <span>1. Phân Biệt Âm &amp; Khẩu Hình</span>
              </button>
              ...
              >
                ...
                <span>2. Đóng Vai Tình Huống</span>
              </button>
...
export default function SpeakingPage() {
  return (
    <Suspense fallback={<div className="min-h-screen bg-[#faf8f5] p-6 space-y-4 max-w-6xl mx-auto" />}>
      <SpeakingContent />
    </Suspense>
  );
}
